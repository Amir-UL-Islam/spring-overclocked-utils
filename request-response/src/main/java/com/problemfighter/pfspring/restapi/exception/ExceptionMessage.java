package com.problemfighter.pfspring.restapi.exception;

public final class ExceptionMessage {
    public static String invalidRequestParams = "Invalid Request Parameter";
    public static String unknownError = "Unknown Error Occur";
    public static String notFound = "Resource Not Found";
    public static String codeError = "Internal Code Error";
    public static String forbidden = "Resource Forbidden";
    public static String badRequest = "Invalid Request";
    public static String conflict = "Resource already exists";
    public static String unauthorized = "You are not allowed";
    public static String validationError = "Data Validation Error";
}
